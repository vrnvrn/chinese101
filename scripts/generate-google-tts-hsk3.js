#!/usr/bin/env node
/*
 * Regenerate June HSK3 (Band 3) Mandarin audio with Google Cloud Text-to-Speech.
 *
 ...
 *
 * Usage:
 *   node scripts/generate-google-tts-hsk3.js
 */
...
const WORDS_FILE = path.join(ROOT, 'refs', 'band3-words.js');
const OUT_DIR = path.join(ROOT, 'audio', 'june', 'hsk3');
const TMP_DIR = path.join(ROOT, 'audio', 'june', `.hsk3-google-tmp-${Date.now()}`);
...
async function main() {
    const words = await loadBand3Words();
