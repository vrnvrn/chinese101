#!/usr/bin/env node
/*
 * Regenerate June HSK1 Mandarin audio with Google Cloud Text-to-Speech.
 *
 ...
 *
 * Usage:
 *   node scripts/generate-google-tts-hsk1.js
 */
...
const WORDS_FILE = path.join(ROOT, 'refs', 'hsk1-words.js');
const OUT_DIR = path.join(ROOT, 'audio', 'june', 'hsk1');
const TMP_DIR = path.join(ROOT, 'audio', 'june', `.hsk1-google-tmp-${Date.now()}`);
...
async function main() {
    const words = await loadHsk1Words();
