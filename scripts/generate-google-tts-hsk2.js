#!/usr/bin/env node
/*
 * Regenerate June HSK2 (Band 2) Mandarin audio with Google Cloud Text-to-Speech.
 *
 ...
 *
 * Usage:
 *   node scripts/generate-google-tts-hsk2.js
 */
...
const WORDS_FILE = path.join(ROOT, 'refs', 'band2-words.js');
const OUT_DIR = path.join(ROOT, 'audio', 'june', 'hsk2');
const TMP_DIR = path.join(ROOT, 'audio', 'june', `.hsk2-google-tmp-${Date.now()}`);
...
async function main() {
    const words = await loadBand2Words();
